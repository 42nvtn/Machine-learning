...
# ╔═╡ dc3cdcb8-2d18-4395-abff-7b2814e8bfe8
begin
	using MLJ
end
...
function get_entropy(y_predict, y)
    ...
    # overall entropy
    #TODO s=?
	l = n_true / n * entropy_true
	r = n_false / n * entropy_false
	s = l + r
    return s
end
...
function Gauss(std, mean, x)
    ...
    #TODO 
    res = (1 / (std * sqrt(2 * pi))) * exp(-(x - mean) ^ 2 / (2 * (std ^ 2)))
	return res
end
...
function fit_1(self::NBGaussian, X,y)
        ...
        for hypo in range(0, length = n_species)
        	...
        	for i in length(y):-1:1
            	if y[i] != hypo
					x = x[setdiff(1:length(x), i), :]
            	end
        	end
			Mean[hypo] = Statistics.mean(x, dims=1)[1]
        	Std[hypo] = Statistics.std(x)
...
# ╔═╡ 7e9985d9-2f5b-4d76-ab3d-7ba0cd77e6a5
begin
	_f1 = multiclass_f1score(pred1, y_test)
	_recall = multiclass_recall(pred1, y_test)
	_precision = multiclass_precision(pred1, y_test)

	print("f1: $(_f1)\nRecall: $(_recall)\nPrecision: $(_precision)\n")
end

# ╔═╡ 39356f78-f170-40a5-a9ac-b84c3a9867e3
md"""
The f1 is high(0.96), means the model has a balance between precision and recall. Recall (0.96) shows that was able to find a large number of true positives. And the precision (0.96) shows that the model did not produce too many false positives.
