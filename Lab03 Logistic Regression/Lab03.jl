...
function compute_average_intensity(x, y)
    ...
    count = zeros(10)
	total = zeros(10)
    for i in 1:size(x_sum)[1]
        total[y[i]+1] += x_sum[i]
        count[y[i]+1] += 1
    end
	for i in 1:10
		mean_[i] = total[i] / count[i]
	end
    return mean_
end
...
begin
	...
	intensity = sum(train_x_flatten,dims=2) ./ size(train_x_flatten, 2)
	size(intensity)
end
...
begin
	...
	train_x_new = cat(intensity, symmetry, dims = (2, 2))
	size(train_x_new)
end
...
function normalize(x, mean_=nothing, std_=nothing)
   if mean_ == nothing && std_ == nothing
    ...
    return normalized_train_x, mean_, std_ #return `normalized_train_x` calculated by using mean_ and std_ (both of them are passed and not null)
end
...
function compute_gradient(error, train_x)
    ...
    """
    gradient = train_x' * sigmoid_deriv(error) / size(train_x, 1)
    return gradient
end
...
begin
	...
	acc = accuracy(preds_train, train_y_n)
	p = precision(preds_train, train_y_n)
	r = recall(preds_train, train_y_n)
	f1 = 2*p*r/(p + r)
...
begin
	...
	test_intensity = sum(test_x_flatten,dims=2) ./ size(test_x_flatten, 2)
	test_symmetry = compute_symmetry(test_x_flatten)
	test_x_new = cat(test_intensity, test_symmetry, dims = (2, 2))
...
begin
	...
	_acc = accuracy(preds_test, test_y_n)
	_p = precision(preds_test, test_y_n)
	_r = recall(preds_test, test_y_n)
	_f1 = 2*_p*_r/(_p + _r)
...
The accuracy and precision is high, but recall and f1_score is low. The model can capture relevant results, but not all of them. Therefore The model still needs improvement 
"""
