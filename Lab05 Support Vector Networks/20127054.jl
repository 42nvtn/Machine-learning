...
student = (name = "Ngo Van Trung Nguyen", id = "20127054")
...
function pla(pos_data::Matrix{Float64}, neg_data::Matrix{Float64}, 
	...
	# START YOUR CODE
	X = vcat(pos_data, neg_data)
    X = hcat(X, ones(size(X, 1)))
    y = vcat(fill(1, size(pos_data, 1)), fill(-1, size(neg_data, 1)))
...
function eval_pla(θ, pos_data, neg_data)
	# START YOUR CODE
	pos_data = hcat(pos_data, fill(1, size(pos_data, 1)))
    neg_data = hcat(neg_data, fill(1, size(neg_data, 1)))

    pos_preds = sign.(pos_data*θ)
    neg_preds = -sign.(neg_data*θ)

    n_correct = sum(pos_preds .== 1) + sum(neg_preds .== -1)
    acc = n_correct / (size(pos_data, 1) + size(neg_data, 1))
...
We have $\\ \theta ^{k+1} =  \theta ^{k} + y^{i}  x^{i}$

We can write:
$\theta ^{k} = \theta ^{0} + \sum_{j=1}^{k} y^{i}  x^{i}$

$(\theta^{*})^\top \theta ^{k} = (\theta^{*})^\top \theta ^{0} + (\theta^{*})^\top \sum_{j=1}^{k} y^{i}  x^{i}$
**END YOUR PROOF**
"""
...
Let's consider two parallel hyperplanes, $\mathbf{H}_1$ and $\mathbf{H}_2$. 

$\mathbf{H}_1: \mathbf{\theta}^\top x + b = 1$ 
$\mathbf{H}_2: \mathbf{\theta}^\top x + b = -1$

Let's take any point $\mathbf{x}_1$ on $\mathbf{H}_1$ and any point $\mathbf{x}_2$ on $\mathbf{H}_2$. The distance between these two points can be calculated:

$\left|\left|\frac{\theta^\top\mathbf{x}_1}{||{\theta}||} - \frac{{\theta}^\top\mathbf{x}_2}{||{\theta}||}\right|\right|$

$= \frac{\left|\theta^\top (\mathbf{x}_1 - \mathbf{x}_2) \right|}{\left|\left|\theta\right|\right|}$

$= \frac{\left|\theta^\top \mathbf{x}_1 + b - (\theta^\top \mathbf{x}_2 +b)\right|}{\left|\left|\theta\right|\right|}$

$= \frac{\left|1 - (-1)\right|}{\left|\left|\theta\right|\right|}$

$= \frac{2}{\left|\left|\theta\right|\right|}$
...
function hardmargin_svm(pos_data, neg_data, η=0.04, n_epochs=10000)
    ...
    w = zeros(size(pos_data, 1))
    b = 0.0

    ## Loss function
    function hinge_loss(x, y, w, b)
    ...
    for epoch in 1:n_epochs
        for i in eachcol(pos_data)
            if dot(w, i) - b < 1
                w += η * i
                b -= η
            end
        end
        for i in eachcol(neg_data)
            if dot(w, i) - b > -1
                w -= η * i
                b += η
            end
...
function softmargin_svm(pos_data, neg_data, n_epochs=10000, C=0.12, η=0.01)
    ...
    for epoch in 1:n_epochs
        for i in 1:(size(pos_data, 2) + size(neg_data, 2))
            if i <= size(pos_data, 2)
                x = pos_data[:, i]
                y = 1.
            else
                x = neg_data[:, i - size(pos_data, 2)]
                y = -1.
            end
            if hinge_loss(x, y) > 0
				ς = 1
